...
interface RecentlyResponse {
    ...
    interface HistoryResponseMapper {
        ...
        class Base(
            ...
            override fun mapError(msg: String) = observable.update(RecentlyState.Error(msg))
        }
    }

    class RecentlyResponseSuccess(private val list: List<RecentlyUi>) : RecentlyResponse {
        override fun map(mapper: HistoryResponseMapper) = mapper.mapSuccess(list)
    }

    class RecentlyResponseError(private val msg: String) : RecentlyResponse {
        override fun map(mapper: HistoryResponseMapper) = mapper.mapError(msg)
    }
}